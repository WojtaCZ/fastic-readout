...
### Communication 
The readout implements two interfaces for communication with the device. The first one is a CDC interface, that emulates a COM port over USB. This interface implements a simple human-readable protocol that allows the user to control all the necessary functions of the device. The following commands are available, where square braces indicate a choice from one of the options, curly braces indicate an expected value and triagular braces indicate an optional flag.
